...
@Component({
  selector: 'app-google-password-reset',
  standalone: true,
  imports: [ RippleModule, ReactiveFormsModule, ProgressBarModule, RouterLink, CalendarModule, ButtonModule, CardModule, CommonModule, FloatLabelModule, InputTextModule, InputGroupModule, InputGroupAddonModule ],
  templateUrl: './google-password-reset.component.html',
  styleUrl: './google-password-reset.component.css'
})
export class GooglePasswordResetComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.finishRegisterForm = this.fb.group({
  ...
  get password() { return this.finishRegisterForm.get('password'); }
  get username() { return this.finishRegisterForm.get('username'); }
  get dateOfBirth() { return this.finishRegisterForm.get('date_of_birth'); }
  get phone() { return this.finishRegisterForm.get('phone'); }
  onSubmit() {
    if (this.finishRegisterForm.valid) {
      ...
      this._api?.post("api/User/change-password-google", {
        user: {
